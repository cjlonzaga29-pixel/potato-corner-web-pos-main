...
**Last verified:** 2026-07-20 — see sync note immediately below and `docs/SYSTEM_OVERVIEW.md` for current architecture; body of this report (§1–§22) is not re-audited past the 2026-07-18 note.
...
> **2026-07-20 documentation sync note:** unplanned follow-on work has landed since the 2026-07-18 note below, not yet reflected in the body of this report: **Phase 20.5** (refresh-token race fix — Postgres advisory lock + rotation-result cache, commit `9507200`), **Phase 21** (full Redis/Upstash/BullMQ eradication — migrated to Postgres-native `pg-lock.ts`/`id-counter.ts`/`job-runner.ts`, commit `28a2956`), and **Phase 21.5** (restored a Postgres rotation-result cache that Phase 21 had inadvertently dropped, commit `6116ff1`). **Every Redis/Upstash/BullMQ reference elsewhere in this document (e.g. §7, §10, §14, §19) now describes removed infrastructure — treat as historical, not current state.** See `docs/SYSTEM_OVERVIEW.md` for the current architecture.
...
Architecture and business rules are locked documents (`docs/architecture/final-approved-architecture.md`, scored **7.8/10 (≈8.5/10 post-fixes)** at sign-off) — "nothing here is open for discussion without a formal change request." Four change requests have since been approved and layered on top of the original spec: CR-001 (branch-level product requests + price overrides), CR-002 (flavor requests), CR-003 (Branch Operating System — employee status lifecycle, branch-authorized staff logins), and CR-004 (POS deduction integrity — cross-branch ingredient resolution, recipe versioning, idempotent branch provisioning, InventoryMovement/Transaction immutability; see `docs/decisions/CR-004-pos-deduction-integrity.md`).
