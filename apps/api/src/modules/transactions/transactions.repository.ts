...
interface CreateTransactionRow {
  ...
  items: {
    // Pre-generated by the service layer so it can target the same row by
    // id when writing the deduction snapshot immediately afterward. Optional
    // for backward compatibility with existing callers/tests — Prisma falls
    // back to its own default(uuid()) when omitted.
    id?: string;
    productId: string;
    productVariantId: string;
    flavorId: string | null;
