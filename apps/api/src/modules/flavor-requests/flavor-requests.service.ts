import { ROLES, SOCKET_EVENTS, type JwtPayload } from '@potato-corner/shared';
import { flavorRequestsRepository } from './flavor-requests.repository.js';
import { FlavorRequestError, type FlavorRequestListFilters } from './flavor-requests.types.js';
import { flavorsRepository } from '../flavors/flavors.repository.js';
import { recordAuditLog } from '../../middleware/audit-log.js';
import { notifySuperAdmin, notifyBranch } from '../../lib/notify.js';

type ActorContext = { id: string; role: string };

interface RequestRow {
  id: string;
  branchId: string;
  requestedBy: string;
  proposedName: string;
  proposedDescription: string | null;
  proposedColorHex: string;
  proposedDisplayOrder: number | null;
  requestReason: string;
  status: string;
  reviewedBy: string | null;
  reviewedAt: Date | null;
  reviewNotes: string | null;
  createdFlavorId: string | null;
  createdAt: Date;
  updatedAt: Date;
  branch: { id: string; name: string; code: string };
  requester: { id: string; firstName: string; lastName: string };
  reviewer: { id: string; firstName: string; lastName: string } | null;
}

function toResponse(row: RequestRow) {
  return {
    id: row.id,
    branch_id: row.branchId,
    branch_name: row.branch.name,
    requested_by: row.requestedBy,
    requested_by_name: `${row.requester.firstName} ${row.requester.lastName}`,
    proposed_name: row.proposedName,
    proposed_description: row.proposedDescription,
    proposed_color_hex: row.proposedColorHex,
    proposed_display_order: row.proposedDisplayOrder,
    request_reason: row.requestReason,
    status: row.status,
    reviewed_by: row.reviewedBy,
    reviewed_by_name: row.reviewer ? `${row.reviewer.firstName} ${row.reviewer.lastName}` : null,
    reviewed_at: row.reviewedAt?.toISOString() ?? null,
    review_notes: row.reviewNotes,
    created_flavor_id: row.createdFlavorId,
    created_at: row.createdAt.toISOString(),
    updated_at: row.updatedAt.toISOString(),
  };
}

interface CreateFlavorRequestInput {
  branch_id: string;
  proposed_name: string;
  proposed_description?: string;
  proposed_color_hex: string;
  proposed_display_order?: number;
  request_reason: string;
}

interface ReviewFlavorRequestInput {
  action: 'approve' | 'reject';
  review_notes?: string;
  overrides?: {
    proposed_name?: string;
    proposed_description?: string;
    proposed_color_hex?: string;
    proposed_display_order?: number;
  };
}

export const flavorRequestsService = {
  /** Supervisor-only. branch_id must be one of the actor's own assigned branches. */
  async submitRequest(data: CreateFlavorRequestInput, actor: JwtPayload, ipAddress: string | null) {
    if (actor.role !== ROLES.SUPERVISOR) {
      throw new FlavorRequestError('INSUFFICIENT_PERMISSIONS', 'Only supervisors may submit flavor requests', 403);
    }
    if (!actor.branch_ids.includes(data.branch_id)) {
      throw new FlavorRequestError('BRANCH_ACCESS_DENIED', 'You may only submit requests for your own branch', 403);
    }

    const created = (await flavorRequestsRepository.create(
      {
        branchId: data.branch_id,
        proposedName: data.proposed_name,
        proposedDescription: data.proposed_description,
        proposedColorHex: data.proposed_color_hex,
        proposedDisplayOrder: data.proposed_display_order,
        requestReason: data.request_reason,
      },
      actor.user_id,
    )) as RequestRow;
    const response = toResponse(created);

    await recordAuditLog({
      action: 'FLAVOR_REQUEST_SUBMITTED',
      entityType: 'flavor_request',
      entityId: created.id,
      actorId: actor.user_id,
      actorRole: actor.role,
      branchId: data.branch_id,
      afterState: response,
      ipAddress,
    });

    notifySuperAdmin(SOCKET_EVENTS.FLAVOR_REQUEST_SUBMITTED, response);

    return response;
  },

  /** super_admin sees everything; supervisor is scoped to their own branch_ids regardless of query filters. */
  async listRequests(actor: JwtPayload, filters: FlavorRequestListFilters) {
    const scoped: FlavorRequestListFilters =
      actor.role === ROLES.SUPERVISOR
        ? { ...filters, branch_id: filters.branch_id && actor.branch_ids.includes(filters.branch_id) ? filters.branch_id : actor.branch_ids[0] }
        : filters;

    const { requests, total } = await flavorRequestsRepository.findAll(scoped);
    return {
      requests: (requests as RequestRow[]).map(toResponse),
      total,
      page: filters.page,
      limit: filters.limit,
    };
  },

  async getRequestById(id: string, actor: JwtPayload) {
    const request = (await flavorRequestsRepository.findById(id)) as RequestRow | null;
    if (!request) throw new FlavorRequestError('FLAVOR_REQUEST_NOT_FOUND', 'Flavor request not found', 404);
    if (actor.role === ROLES.SUPERVISOR && !actor.branch_ids.includes(request.branchId)) {
      throw new FlavorRequestError('BRANCH_ACCESS_DENIED', 'You may only view requests for your own branch', 403);
    }
    return toResponse(request);
  },

  async reviewRequest(id: string, data: ReviewFlavorRequestInput, actor: ActorContext, ipAddress: string | null) {
    const request = (await flavorRequestsRepository.findById(id)) as RequestRow | null;
    if (!request) throw new FlavorRequestError('FLAVOR_REQUEST_NOT_FOUND', 'Flavor request not found', 404);
    if (request.status !== 'pending') {
      throw new FlavorRequestError('FLAVOR_REQUEST_ALREADY_REVIEWED', 'This request has already been reviewed', 409);
    }

    if (data.action === 'reject') {
      const updated = (await flavorRequestsRepository.updateStatus(id, {
        status: 'rejected',
        reviewedBy: actor.id,
        reviewNotes: data.review_notes,
      })) as RequestRow;
      const response = toResponse(updated);

      await recordAuditLog({
        action: 'FLAVOR_REQUEST_REJECTED',
        entityType: 'flavor_request',
        entityId: id,
        actorId: actor.id,
        actorRole: actor.role,
        branchId: request.branchId,
        beforeState: toResponse(request),
        afterState: response,
        ipAddress,
      });

      notifyBranch(request.branchId, SOCKET_EVENTS.FLAVOR_REQUEST_REVIEWED, response);
      return response;
    }

    // Approve: create the catalog flavor from the (optionally admin-edited)
    // proposal, reusing flavorsRepository.create rather than duplicating
    // flavor-creation logic.
    const finalName = data.overrides?.proposed_name ?? request.proposedName;
    const finalDescription = data.overrides?.proposed_description ?? request.proposedDescription ?? undefined;
    const finalColorHex = data.overrides?.proposed_color_hex ?? request.proposedColorHex;
    const finalDisplayOrder = data.overrides?.proposed_display_order ?? request.proposedDisplayOrder ?? undefined;

    const existing = await flavorsRepository.findByName(finalName);
    if (existing) {
      throw new FlavorRequestError('FLAVOR_NAME_CONFLICT', `A flavor named "${finalName}" already exists`, 409);
    }

    const createdFlavor = await flavorsRepository.create({
      name: finalName,
      description: finalDescription,
      colorHex: finalColorHex,
      displayOrder: finalDisplayOrder,
      isActive: true,
    });

    const updated = (await flavorRequestsRepository.updateStatus(id, {
      status: 'approved',
      reviewedBy: actor.id,
      reviewNotes: data.review_notes,
      createdFlavorId: createdFlavor.id,
    })) as RequestRow;
    const response = toResponse(updated);

    await recordAuditLog({
      action: 'FLAVOR_REQUEST_APPROVED',
      entityType: 'flavor_request',
      entityId: id,
      actorId: actor.id,
      actorRole: actor.role,
      branchId: request.branchId,
      beforeState: toResponse(request),
      afterState: { ...response, createdFlavorId: createdFlavor.id },
      ipAddress,
    });

    notifyBranch(request.branchId, SOCKET_EVENTS.FLAVOR_REQUEST_REVIEWED, response);
    return response;
  },
};
